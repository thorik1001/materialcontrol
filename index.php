...
$barang=mysql_num_rows(mysql_query("SELECT * FROM barang"));
$supplier=mysql_num_rows(mysql_query("SELECT * FROM supplier"));
$pelanggan=mysql_num_rows(mysql_query("SELECT * FROM pelanggan"));
$beli=mysql_num_rows(mysql_query("SELECT * FROM beli"));
$jual=mysql_num_rows(mysql_query("SELECT * FROM jual"));
$account=mysql_num_rows(mysql_query("SELECT * FROM account"));
$prreq=mysql_num_rows(mysql_query("SELECT * FROM tblpritem"));

$sumBeli=mysql_fetch_array(mysql_query("SELECT SUM(total) AS totalBeli FROM beli"));
$sumjual1=mysql_fetch_array(mysql_query("SELECT SUM(total) AS totalJual1 FROM jual"));
$sumjual2=mysql_fetch_array(mysql_query("SELECT SUM(biaya_kirim) AS totalJual2 FROM jual"));
$Tsumjual=$sumjual1[totalJual1]+$sumjual2[totalJual2];

$GT=$sumBeli[totalBeli]+$Tsumjual;
$PL=$Tsumjual-$sumBeli[totalBeli];


$persenBeli=round($sumBeli[totalBeli]/$GT*100);
$persenjual=round($Tsumjual/$GT*100);
?>
<!DOCTYPE html>
<html lang="en">
<head>
	...
	<!-- start: Meta -->
	<meta charset="utf-8">
	<title>::.<?php echo"$idn[nama_identitas]";?>.::</title>
	<meta name="description" content="Bootstrap Metro Dashboard">
	<meta name="author" content="Muhamad Thorik">
	<!-- end: Meta -->
...
<!--END HEADER MENU ATAS -->		
	...
	<div class="modal hide fade" id="myModal">
		...
		<div class="modal-body">
			<p>(c)Creativityright by Muhamad Thorik</p>
		</div>
	...
	<footer>
		...
		<p>
			<span style="text-align:left;float:left">&copy; <?php echo"$tahun";?> - Creativityright by Muhamad Thorik untuk | <a href="#" alt="<?php echo"$idn[nama_identitas]";?>"><?php echo"$idn[nama_identitas]";?></a></span>
...
function charts() {
	...
	/* ---------- Pie chart ---------- */
	var data = [
	{ label: "Pembelian",  data: <?php echo"$persenBeli"; ?>},
	{ label: "Penjualan",  data: <?php echo"$persenjual"; ?>}
	];
